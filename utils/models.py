...
def init_transformers_model(model_class, model_path, tokenizer, device_map, load_in_8bit):
    if load_in_8bit:
...
def get_transformers_model(base_model, tokenizer, model_class=AutoModelForCausalLM, lora_path="", lora_r=0, lora_alpha=16, lora_dropout=0.05, lora_target_modules=None, device_map="auto", load_in_8bit=False):
    model = init_transformers_model(model_class, base_model, tokenizer, device_map, load_in_8bit)
...
def get_dpo_model(model_path, tokenizer, lora_r=0, lora_alpha=16, lora_dropout=0.05, lora_target_modules=None, device_map="auto", load_in_8bit=False):
    ref_model = init_transformers_model(AutoModelForCausalLM, model_path, tokenizer, device_map, load_in_8bit)
...
def get_qadapter_model(model_path, tokenizer, alpha, omega, gamma, beta, lora_path="", lora_r=0, lora_alpha=16, lora_dropout=0.05, lora_target_modules=None, device_map="auto", load_in_8bit=False, register_callback=False):
    model = init_transformers_model(AutoModelForCausalLM, model_path, tokenizer, device_map, load_in_8bit)
    ...
    result = modify_forward(model, alpha_tilde=alpha, alpha_0=omega, gamma=gamma, beta=beta, register_callback=register_callback)
    return result
