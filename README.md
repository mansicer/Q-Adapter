...
The implementation of the paper "Q-Adapter: Training Your LLM Adapter as a Residual Q-Function", [arXiv link](https://arxiv.org/abs/2407.03856). This repo is adopted from the [alpaca-lora](https://github.com/tloen/alpaca-lora) repo. 
...
CUDA_VISIBLE_DEVICES=0,1,2,3 TOKENIZERS_PARALLELISM=true accelerate launch --config_file accelerate_config.yaml qadapter_train.py \
--base_model=meta-llama/Meta-Llama-3-8B-Instruct --dataset_name=hh-rlhf --data_dir=${data_class} \
--logging_steps=20 --eval_steps=100 --num_epochs=3 --micro_batch_size=1 \
--wandb_project=Q-Adapter --wandb_run_name=QAdapter-${data_class} --output_dir=logs/qadapter-${data_class}
```
## Citation

If you find this repo helpful, please consider citing the following paper:
